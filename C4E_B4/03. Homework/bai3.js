...
console.log(jobData.hits) // hits = array

console.log(jobData.hits[0]) // Tìm đến arry có index = 0, là 1 object chứa nhiều cặp key value

console.log(jobData.hits[0].jobTitle) // jobTitle: là key của ob thuộc phần tử array hits có index = 0

console.log(jobData.hits[0].benefits) // benefits: là key của ob và là 1 array chứa 3 phần tử, mỗi phần tử là 1 ob

for (i in jobData.hits[0].benefits) {
    console.log(jobData.hits[0].benefits[i].benefitValue)
}

// Xét lần lượt từng phần tử trong mảng benefits
// In ra lần lượt từng value của key benefitValue trong từng đối tượng là phần tử của mảng benefits
...
for (i in jobData.hits) {
    ...
    for (n in jobData.hits[i].benefits) {     

        console.log(jobData.hits[i].benefits[n].benefitValue)

        console.log(jobData.hits[i].jobTitle)
        console.log(jobData.hits[i].locations)
        console.log(jobData.hits[i].skills)
...
// In ra jobTitle: Xét từng phần tử của array hits theo index 
//                 -> Truy cập vào từng phần tử của array hits là các Object để lấy ra value jobTitle

// in ra benefitsValue của tất cả job: Tiếp tục dùng vòng lặp for để xét từng phần tử của array con benefits thuộc arraycha hits
//                 -> Truy cập vào từng phần tử của array con là các Object để lấy ra giá trị benefitsValue
