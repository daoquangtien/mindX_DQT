...
let reloadAction = document.getElementById("quote--content__reloadQuote")



reloadAction.onclick = function() {

    let randomIndex = Math.floor(Math.random() * rdQuote.length - 1)

    // Nội dung quote
    let quoteContent = document.getElementById("quote--content__randomQuote")
    quoteContent.innerText = rdQuote[randomIndex]["quoteText"]

    // Tên tác giả
    let quoteAuthor = document.getElementById("quote--content__randomAuthor")
    quoteAuthor.innerText = rdQuote[randomIndex]["quoteAuthor"]
